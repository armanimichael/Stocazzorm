...
BenchmarkRunner.Run<StocazzormBenchmarks>();

[MemoryDiagnoser]
public class StocazzormBenchmarks
{
    ...
    [Benchmark]
    public async Task<Todo?> StocazzormDbParameters()
    {
        const string sql = """
            INSERT INTO Todos(Title, IsComplete)
            Values($1, $2)
    ...
    [Benchmark]
    public async Task<Todo?> StocazzormStringInterpolation()
    {
        var createdTodo = await _dataSource.QuerySingleAsync<Todo>($"""
            INSERT INTO Todos(Title, IsComplete)
    ...
    [Benchmark]
    public async Task<Todo?> StocazzormDbCommonParameters()
    {
        const string sql = """
            INSERT INTO Todos(Title, IsComplete)
            Values($1, $2)
    ...
    [Benchmark]
    public async Task<Todo?> StocazzormDbCommonStringInterpolation()
    {
        var dataSource = (DbDataSource)_dataSource;
        var createdTodo = await dataSource.QuerySingleAsync<Todo>($"""
            INSERT INTO Todos(Title, IsComplete)
